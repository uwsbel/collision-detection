...
int main(int argc, char** argv) {
	...
	for (int i = 0; i < collisionListB.size(); i++) {
		...
		for (int j = 0; j < collisionListA.size() && collisionFound == false; j++) {
			if ((collisionListB[i].x == collisionListA[j].x && collisionListB[i].y == collisionListA[j].y)
					|| (collisionListB[i].x == collisionListA[j].y && collisionListB[i].y == collisionListA[j].x)) {
				collisionFound = true;
			}
		}
